...
class Grid():
    ...
    def updateTrafoInformation(self):   
        ...
        # det(A) = ad-bc
        # abs as np.abs
        self.adet = np.abs(np.multiply(verts[:,0,1]-verts[:,0,0],verts[:,1,2]-verts[:,1,0]) - np.multiply(verts[:,0,2]-verts[:,0,0],verts[:,1,1]-verts[:,1,0]))
        ...
        # inverse 
        self.invJac = np.divide(adjMat,self.adet[:,np.newaxis,np.newaxis])
